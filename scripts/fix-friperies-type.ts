...
if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
  console.error(
    '❌ Missing Supabase credentials (need SERVICE_ROLE_KEY for admin operations)'
  );
  process.exit(1);
}
...
async function main() {
  ...
  const candidatesForFriperie = ressourceries.filter((r) => {
    const isSynthetic =
      r.name.includes('#') || r.name.match(/Ressourcerie (du|de|des) .+ - #/);
  ...
  console.log(
    `🎯 Candidats pour conversion en "Friperie": ${candidatesForFriperie.length}`
  );
  ...
  console.log(
    `   ${candidatesForFriperie.length} initiatives seront modifiées`
  );
  console.log();
  ...
  for (let i = 0; i < idsToUpdate.length; i += batchSize) {
    ...
    if (error) {
      console.error(
        `❌ Erreur lors de la mise à jour du lot ${Math.floor(i / batchSize) + 1}:`,
        error.message
      );
      continue;
    }
    ...
    process.stdout.write(
      `\r   ${totalUpdated} / ${idsToUpdate.length} convertis...`
    );
  }
  ...
  console.log();
  console.log(
    '🗺️  Recharge la carte pour voir les friperies avec leur icône rose 💗'
  );
}
